...
export const insertBookPublicationModel = async (bookPublicationData: BookPublicationDetails) => {
   ...
   const data =
      await sql`SELECT * FROM insert_book_publication(${JSON.parse(JSON.stringify(bookPublicationData))}, '1');`;
   return data;
};
...
export const updateBookPublicationModel = async (bookPublicationData: BookPublicationDetails) => {
   ...
   const data =
      await sql`SELECT * FROM upsert_book_publication(${JSON.parse(JSON.stringify(bookPublicationData))}, '1');`;
   return data;
};
...
export const deleteBookPublicationModel = async (bookPublicationId: number) => {
   ...
   const data =
      await sql`UPDATE book_publication SET active = false,modified_date=now(),modified_by='1' WHERE id = ${bookPublicationId}`;
